...
{
    /// <summary>
    /// Memory.dll class. Full documentation at (Removed Link for UC reasons)
    /// </summary>
    public class Mem
    {
        #region DllImports
        ...
        [DllImport("kernel32.dll")]
        static extern bool WriteProcessMemory2(
            IntPtr hProcess,
            UIntPtr lpBaseAddress,
            string lpBuffer,
            UIntPtr nSize,
            out IntPtr stringWritten
        );
        ...
        public void FreezeValue(string address, string type, string value, int length = 0, string file = "")
        {
            ...
            Task.Factory.StartNew(() =>
            {
                ...
                {
                    WriteMemory(address, type, value, length, file);
                    Thread.Sleep(25);
                }
            },
            cts.Token);
        ...
        public bool WriteMemory(string code, string type, string write, [Optional] int length, string file = "", System.Text.Encoding stringEncoding = null, bool RemoveWriteProtection = true)
        {
            byte[] memory = new byte[4];
            int size = 4;
            ...
            else if (type.ToLower() == "unicode")
            {
                if (stringEncoding == null)
                    memory = Encoding.Unicode.GetBytes(write);
                else
                    memory = stringEncoding.GetBytes(write);
                size = length;
